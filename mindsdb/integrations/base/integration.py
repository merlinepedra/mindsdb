                    ...
                    if 'action' not in dct:
                        ...
                        if dct['action'] == 'create':
                            for k in ['name', 'predictor', 'stream_in', 'stream_out']:
                                ...
                                if db.session.query(db.Stream).filter_by(name=dct['name'],
                                                                         company_id=self.company_id).first() is None:
                                    stream = db.Stream(
                                    ...
                                    log.error('INTEGRATION %s: stream with this name already exists - %s', self.name,
                                              dct['name'])
                            ...
                            for k in ['name']:
                                if k not in dct:
                                    # Not all required parameters were provided (i.e. stream will not be created)
                                    # TODO: what's a good way to notify user about this?
                                    log.error('INTEGRATION %s: unable to delete stream - stream name is not provided',
                                              self.name)
                                    break
                            else:
